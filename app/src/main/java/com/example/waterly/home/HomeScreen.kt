...
fun HomeScreen(
                        ...
                        Image(
                            ...
                            modifier = Modifier
                                ...
                                .offset {
                                    IntOffset(
                                        0,
                                        if (uiState.selectedItem == "glass") (-8).dp.roundToPx() else 0
                                    )
                                }
                                .clickable @androidx.annotation.RequiresPermission(android.Manifest.permission.POST_NOTIFICATIONS) {
                                    viewModel.onGlassClicked(
                                        context
                                    )
                                },
                            colorFilter = if (uiState.selectedItem != "glass") ColorFilter.tint(
                                Color.Gray
                            ) else null
                        )
                        ...
                        Image(
                            ...
                            modifier = Modifier
                                ...
                                .offset {
                                    IntOffset(
                                        0,
                                        if (uiState.selectedItem == "bottle") (-8).dp.roundToPx() else 0
                                    )
                                }
                                .clickable { viewModel.onBottleClicked(context) },
                            colorFilter = if (uiState.selectedItem != "bottle") ColorFilter.tint(
                                Color.Gray
                            ) else null
                        )
                    }
    ...
    // 🎉 Confetti animation overlay (top of all content)
    if (uiState.showConfetti) {
        val composition by rememberLottieComposition(
            LottieCompositionSpec.Asset("confettiAnimation.json")
        )
        ...
        LottieAnimation(
            ...
            modifier = Modifier
                .fillMaxSize()
                .padding(top = 60.dp) // fine-tune height to float above the water circle
